    ...
    render() {
        return (
            <tr>
                <td>
                    <NavLink
                        to={{ pathname: "/JLDetail", query: { _id: this.props._id } }}
                    >
...
class JLForm extends Component {
    state = {
        jlList: [],
        isLogin: 0
    };
    ...
    getJLList = () => {
        ...
        axios
            .post("http://localhost:8080/jl/getJLList", send_param)
            .then(returnData => {
                let jlList;
                console.log(returnData.data.list.length);
                if (returnData.data.list.length > 0) {
                    //console.log(returnData.data.list.length);
                    const jls = returnData.data.list;
                    jlList = jls.map(item => (
                        <JLRow
    ...
    render() {
        ...
        return (
            <div >
                <div style={{ backgroundColor : '#F0EAE3', fontFamily:'MapoGoldenPier', color: '#3A1900',padding:'0'}}>
                    <h3 style={{fontSize:'23px'}}>&nbsp; &nbsp; &nbsp; &nbsp;&nbsp;여기는 전라도 입니다.</h3>
