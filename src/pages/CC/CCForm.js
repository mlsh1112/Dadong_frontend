    ...
    render() {
        return (
            <tr>
                <td>
                    <NavLink
                        to={{ pathname: "/CCDetail", query: { _id: this.props._id } }}
                    >
...
class CCForm extends Component {
    ...
    getCCList = () => {
        ...
        axios
            .post("http://localhost:8080/cc/getCCList", send_param)
            .then(returnData => {
                let ccList;
                console.log(returnData.data.list.length);
                if (returnData.data.list.length > 0) {
                    //console.log(returnData.data.list.length);
                    const ccs = returnData.data.list;
                    ccList = ccs.map(item => (
                        <CCRow
    ...
    render() {
        ...
        return (
            <div >
                <div style={{ backgroundColor : '#F0EAE3', fontFamily:'MapoGoldenPier', color: '#3A1900',padding:'0'}}>
                    <h3 style={{fontSize:'23px'}}>&nbsp; &nbsp; &nbsp; &nbsp;&nbsp;여기는 충청도 입니다.</h3>
